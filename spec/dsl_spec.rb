...
=begin
describe 'Definitions within an events.rb file' do
  ...
  specify 'calling each() on a registrar defines a RegisteredEventCallback' do
  ...
  specify 'the each() method does not exist on the object returned by events()' do
    the_following_code {
      load_code_from_mocked_filesystem %{ events.each {} }
    }.should.raise NoMethodError
  end
  
  specify 'addressing a non-existent path raises an Exception' do
...
describe 'Executing synchronous events defined within an events.rb file' do
  
  before :each do
    Adhearsion::Events.reinitialize_framework_events_container!
  end
  
  specify 'an exception in a callback should be passed to ahn_log.events.error' do
  ...
  specify 'events should execute the callbacks in the order in which they were defined' do
    order_keeper = []
...
describe 'Executing asynchronous events defined within an events.rb file' do
  
end

describe "Defining new namespaces and events within an EventsDefinitionContainer's object graph" do
  ...
  specify 'initializes a RootNamespace' do
    container.root.should.be.kind_of Adhearsion::Events::RootEventNamespace
  end
  
  specify 'allows the registration of new namespaces' do
  ...
  specify 'the callback is executed with the proper message' do
...
describe 'NamespaceDefinitionCapturer' do
  ...
  specify 'captures any method and returns a NamespaceDefinitionCapturer' do
