...
describe "The lifecycle of an Invocation" do
  ...
  it "should have an initial state of :new" do
    Theatre::Invocation.new(@block).current_state.should eql(:new)
  end
  
  it "should not have a @queued_time until state becomes :queued" do
    invocation = Theatre::Invocation.new(@block)
  ...
  it "should have a valid guid when instantiated" do
    Theatre::Invocation.new(@block).unique_id.should =~ /^[a-z0-9]{8}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{12}$/i
  end
